...
/* -(  Loading Objects  )---------------------------------------------------- */
  ...
  public function load($id) {
    ...
    return $this->loadOneWhere(
      '%C = %d',
      $this->columnNameFromPropertyName($this->getIDKeyForUse()),
      $id);
  }
  ...
  public function reload() {
    ...
    if (!$use_locks) {
      ...
      $result = $this->loadOneWhere(
        ...
        $this->columnNameFromPropertyName($this->getIDKeyForUse()),
        $this->getID(),
        $this->columnNameFromPropertyName('version'),
        $this->getVersion());
    }

    if (!$result) {
      throw new PhutilQueryObjectMissingException($use_locks);
    }
  ...
  public function loadFromArray(array $row) {
    $map = array();
    foreach ($row as $k => $v) {
      $k = $this->propertyNameFromColumnName($k);
      $map[$k] = $v;
    }
  ...
  public function loadAllFromArray(array $rows) {
    ...
    foreach ($rows as $row) {
      $obj = clone $this;
      if ($id_key) {
        $result[$row[$this->columnNameFromPropertyName($id_key)]]
...
/* -(  Writing Objects  )---------------------------------------------------- */
  ...
  public function update() {
    ...
    foreach ($data as $k => $v) {
      ...
      $map[$this->columnNameFromPropertyName($k)] = $v;
    }
    ...
    if ($use_locks) {
      $conn->query(
        'UPDATE %T SET %U, version = version + 1 WHERE %C = %d AND %C = %d',
        $this->getTableName(),
        $map,
        $this->columnNameFromPropertyName($this->getIDKeyForUse()),
        $this->getID(),
  ...
  protected function insertRecordIntoDatabase($mode) {
    ...
    foreach ($columns as $k => $property) {
      $columns[$k] = $this->columnNameFromPropertyName($property);
    }
